...
def decrypt_message_try_pads(ciphertext, pads):
    ...
    plain_message = []
    for x in pads:
        message.append(ps4b.EncryptedMessage.decrypt_message(ciphertext, x))
    
    for i in message:
        plain_message.append(str.split(ps4b.Message.get_text(i)))
...
def decode_story():
    ...
    text_story = ps4b.PlaintextMessage.get_text(decrypt_message_try_pads(story, pads))
